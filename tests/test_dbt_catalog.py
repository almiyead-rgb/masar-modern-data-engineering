"""Unit checks for catalog metadata plumbing; not a claim of native execution."""
import sys
...
class CatalogTests(unittest.TestCase):
    def test_uses_structured_native_schema(self):
        adapter = Adapter()
        rows = list(describe_catalog_columns(adapter, 'example.silver_trips'))
    ...
    def test_empty_schema_fails_instead_of_inventing_columns(self):
        adapter = Adapter()
        adapter.get_columns_in_relation = lambda relation: []
        with self.assertRaisesRegex(ValueError, 'No actual columns'):
            list(describe_catalog_columns(adapter, 'missing'))
    ...
    def test_context_restores_the_original_method(self):
        ...
        with structured_spark_catalog(Adapter):
            ...
            self.assertEqual(len(list(Adapter()._get_columns_for_catalog('table'))), 1)
